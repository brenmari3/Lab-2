...
while guess_count <= max_guesses:
    ...
    def fun(win):
        print(f"You are {win} at guessing!")
    def sad(lose):
        print(f"You are {lose} at guessing :(")
    if MIN <= guess <= MAX:
        if guess == win_num:
            print("You win!")
            fun('awesome')
            break
        elif guess in [win_num + 1, win_num + 2, win_num +3, win_num - 1, win_num -2,  win_num -3] and bonus == False:
            max_guesses += 1
            print("You're close! Here's an extra guess.")
            print(f"Your current guess is {guess}. You are on guess #{guess_count} of {max_guesses}.")
            bonus = True
        elif guess in [win_num + 1, win_num + 2, win_num +3, win_num - 1, win_num -2,  win_num -3]:
            print("You're close!")
            print(f"Your current guess is {guess}. You are on guess #{guess_count} of {max_guesses} ")
